...
app.get("/fundsmanagement", authenticateUser, async (req, res) => {
  try {
    const existingUser = await mainUserData.findOne({ username: req.user.username });
...
app.get("/requestsplit", authenticateUser, async (req, res) => {
  try {
    ...
    if (!participants || !Array.isArray(participants) || participants.length === 0 || !amount || amount <= 0) {
      return res.status(400).json({ message: "Invalid input" });
    }

    const requester = await mainUserData.findOne({ username: req.user.username });
...
app.get("/recharge", authenticateUser, async (req, res) => {
  try {
    ...
    res.json({
      balance: existingUser.balance,
      availableBills: ["Electricity", "Water", "Gas", "Mobile Recharge", "DTH", "Broadband"],
    });
  } catch (error) {
...
app.post("/transaction", authenticateUser, async (req, res) => {
  try {
    ...
    const transaction = {
      amount,
      type: "debit",
      description: type === "recharge" ? `Mobile Recharge (${accountNumber})` : `${billType} Bill Payment`,
      date: new Date(),
    };
    ...
    // Add notification
    const notification = {
      message: `₹${amount} debited for ${type === "recharge" ? `Mobile Recharge (${accountNumber})` : `${billType} Bill Payment`}.`,
      date: new Date(),
      isRead: false,
    };
...
app.get("/wallet", authenticateUser, async (req, res) => {
  try {
    ...
    res.json({ balance: existingUser.balance, history: existingUser.transactions });
  } catch (error) {
    res.status(500).json({ message: "Internal Server Error" });
  }
});
...
app.post("/cashback/redeem", authenticateUser, async (req, res) => {
  try {
    ...
    res.json({ message: `₹${amount} Cashback Redeemed`, cashback: existingUser.cashbackEarned });
  } catch (error) {
    res.status(500).json({ message: "Internal Server Error" });
  }
});
...
app.post("/signin", async (req, res) => {
  try {
    ...
    res.status(200).json({ message: "User signed in successfully", token, route: "/homepage" });
  } catch (error) {
...
app.post("/signup", async (req, res) => {
  try {
    ...
    const mainUser = new mainUserData({ username, balance: 0, transactions: [], cashbackEarned: 0, offers: [], notifications: [] });
    ...
    res.status(201).json({ message: "User created successfully", token, route: "/homepage" });
  } catch (error) {
    if (error instanceof z.ZodError) {
      return res.status(400).json({ message: error.errors });
    }
